...
config = {
    # bookkeeping
    "dataset":                  DATASET,
    "plots_dir":                f"/ceph/lfertig/Thesis/notebook/{DATASET}/plots/Qwen3-4B/FS/NT",
    "unit":                     "days"
}
...
run = wandb.init(
    project=f"Qwen3-4B_NT_FewShot_{config['dataset']}",
    entity="privajet-university-of-mannheim",
    name=f"Qwen3-4B_fs_nt_{ts}",
...
def score_bins(prefix_tokens, last_act=None):
    # Build prompt IDs and run prompt once with cache
    demos = retrieve_demos(prefix_tokens, FS_CFG["n_shots"], FS_CFG["max_demo_events"])
    P_ids = get_prompt_ids(prefix_tokens, demos).unsqueeze(0)  # [1, T]
    pad_id = tokenizer.pad_token_id
    attn = (P_ids != pad_id)

    out = model(input_ids=P_ids, attention_mask=attn, use_cache=True)
    base_past = out.past_key_values  # reuse for each label
