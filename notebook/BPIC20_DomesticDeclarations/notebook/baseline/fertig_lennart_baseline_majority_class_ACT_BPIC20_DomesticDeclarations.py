...
run = wandb.init(
    project="baseline_majority_ACT_BPIC20_DomesticDeclarations",
...
majority_act_prefix_df = pd.DataFrame(prefix_data)
...
train_df = majority_act_prefix_df[majority_act_prefix_df["case_id"].isin(train_ids)].reset_index(drop=True)
val_df   = majority_act_prefix_df[majority_act_prefix_df["case_id"].isin(val_ids)].reset_index(drop=True)
test_df  = majority_act_prefix_df[majority_act_prefix_df["case_id"].isin(test_ids)].reset_index(drop=True)
...
# %%
plot_dir = "/ceph/lfertig/Thesis/notebook/BPIC20_DomesticDeclarations/plots/Baselines/MAJ/ACT"
