# %% Zero-shot (ICL-free) Next-Time prediction (NT) with GPT-Neo-1.3B
...
run = wandb.init(
    project="gpt-neo-1.3B_NT_Zero-Shot_BPI_Challenge_2012C",
...
for case_id, g in df.groupby("case:concept:name", sort=False):
    ...
    # prefixes 1..n-1 → delta between event i and i+1
...
# Global fallback (robust when LM output is unparsable): median delta (minutes) on TRAIN
...
PROMPT_TMPL = (
    "System: You are a process mining assistant. Given a trace of activities, "
    "predict the time until the next event in MINUTES. Reply with ONLY a non-negative number.\n"
    "User: {trace}\n"
    "Assistant: "
)
...
def predict_delta_minutes(prefix_tokens):
    ...
    gen_ids = model.generate(
        ...
        max_new_tokens=8,          # enough for a number
    ...
    # Extract first number (allow decimal); clamp to [0, reasonable_max]
    ...
    if not np.isfinite(minutes):
        return FALLBACK_MINUTES
    # clamp extreme hallucinations; e.g., 30 days in minutes as a hard cap
...
y_pred_all = np.array([predict_delta_minutes(p) for p in test_df["prefix"]], dtype=np.float32)
...
wandb.log({
    "scatter_true_vs_pred": wandb.plot.scatter(tab, "true_min", "pred_min", title="NT: True vs Pred (minutes)"),
    "error_hist": wandb.Histogram(abs_err),
})

# %% Plots → disk (headless)
plot_dir = "/ceph/lfertig/Thesis/notebook/BPI_Challenge_2012C/plots/ZS/NT"
...
# %% Samples table (same schema style you used)
