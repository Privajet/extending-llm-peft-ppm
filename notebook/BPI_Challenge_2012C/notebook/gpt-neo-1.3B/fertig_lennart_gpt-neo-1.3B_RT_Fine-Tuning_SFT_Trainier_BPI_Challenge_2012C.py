# %% RT fine-tuning (SFT + LoRA) with GPT-Neo-1.3B on BPI_Challenge_2012C
...
run = wandb.init(
    project="gpt-neo-1.3B_RT_Fine-Tuning_SFT_Trainer_BPI_Challenge_2012C",
...
# Build RT prefixes: time from current event to case end (minutes)
rows = []
for case_id, g in df.groupby("case:concept:name", sort=False):
    events = g.sort_values("time:timestamp")
    acts  = events["concept:name"].tolist()
    times = events["time:timestamp"].tolist()
    case_end = times[-1]
    for i in range(1, len(acts)):  # prefix len i, remaining from event i-1 to end
        rows.append({
            "case_id": case_id,
            "prefix":  acts[:i],
...
# Temporal split by case start (like ACT)
case_start = (
    df.groupby("case:concept:name")["time:timestamp"]
      .min().reset_index().sort_values("time:timestamp")
)
valid_cases = set(rt_df["case_id"])
...
FALLBACK_MIN = float(np.median(train_df["remaining_time"])) if len(train_df) else 0.0
log.info("Fallback (median train RT minutes): %.2f", FALLBACK_MIN)

# %% SFT strings (numeric-only target after 'Assistant:')
ROUND_TARGET = True  # train on integer minutes; keeps tokenization simple/stable

def make_text(prefix, gold_minutes):
    seq = " → ".join(prefix)
    y = int(round(float(gold_minutes))) if ROUND_TARGET else float(gold_minutes)
    return (
        "System: Predict the remaining time UNTIL CASE COMPLETION in MINUTES. "
        "Reply with ONLY a non-negative number (no text, no units).\n"
        f"User: {seq}\n"
        f"Assistant: {y}"
    )

train_texts = [make_text(p, y) for p, y in zip(train_df["prefix"], train_df["remaining_time"])]
val_texts   = [make_text(p, y) for p, y in zip(val_df["prefix"],   val_df["remaining_time"])]
...
# %% Completion-only collator (mask loss before 'Assistant:')
...
gen_model.eval()
if torch.cuda.is_available(): gen_model = gen_model.to("cuda")
gen_model.config.use_cache = True
device = gen_model.device

# %% Inference prompt (no gold; ends with 'Assistant: ')
PROMPT_TMPL = (
    "System: Predict the remaining time UNTIL CASE COMPLETION in MINUTES. "
    "Reply with ONLY a non-negative number (no text, no units).\n"
    "User: {trace}\n"
...
def _make_prompt(prefix_tokens): return PROMPT_TMPL.format(trace=" → ".join(prefix_tokens))
...
def predict_remaining_minutes(prefix_tokens):
    ...
    if not np.isfinite(minutes):
        return FALLBACK_MIN
    # Clamp extreme values: [0, 30 days]
    return float(np.clip(minutes, 0.0, 60.0 * 24.0 * 30.0))
...
# %% Plots → disk
plot_dir = "/ceph/lfertig/Thesis/notebook/BPI_Challenge_2012C/plots/FT/RT"
...
for _, r in sample.iterrows():
    ...
    table.add_data(r["case_id"], r["k"], " → ".join(r["prefix"]), gold, pred, abs(gold - pred))
...
artifact = wandb.Artifact(name=f"neo_rt_lora_{ts}", type="model")
