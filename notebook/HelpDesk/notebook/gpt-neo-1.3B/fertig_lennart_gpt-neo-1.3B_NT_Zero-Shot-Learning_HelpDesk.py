...
ZS_CFG = {
    ...
    "prompt_tmpl": (
        "Trace: {trace}\n"
        "Predict the time until the next event. Choose EXACTLY ONE label from the list below and output ONLY that label (do not add extra text).\n"
        "Time bins (in days):\n{labels}\n"
        "Answer:"
    ),
...
run = wandb.init(
    project="gpt-neo-1.3B_NT_ZeroShot_HelpDesk",
