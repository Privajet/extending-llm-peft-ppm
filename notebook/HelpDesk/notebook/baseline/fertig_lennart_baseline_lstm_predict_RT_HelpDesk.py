# %% LSTM baseline — Remaining Time (RT) prediction on HelpDesk
...
config = {
    ...
    "min_lr":               1e-6,
    "max_ctx":              30,         # added (20.09.)
    "weight_decay":         1e-4,       # added (20.09.)
...
run = wandb.init(
    project="baseline_lstm_RT_HelpDesk",
...
def prepare_data(frame):
    ...
    # log1p: maps days>=0 → real numbers
    y_log = np.log1p(frame["remaining_time"].astype("float32").to_numpy()).reshape(-1, 1)
    return X, y_log
...
model.compile(
    optimizer=optimizer,
    loss=loss,
    metrics=[tf.keras.metrics.MeanAbsoluteError(name="mae")]   # on scaled targets
)
...
# %% Plots → disk
plot_dir = "/ceph/lfertig/Thesis/notebook/HelpDesk/plots/Baselines/LSTM/RT"
...
artifact = wandb.Artifact("lstm_rt_model_helpdesk", type="model")
