# %% N-gram baseline — Remaining-Time (RT) on HelpDesk
# - Temporal split by case start (same protocol as your ACT/NT scripts)
# - For each prefix of length k (k=1..n-1), target is remaining time R_k = t_end - t_{k-1} in days
# - n-gram stores mean remaining time per context (last n-1 activities)
...
run = wandb.init(
    project="baseline_ngram_RT_HelpDesk",
...
# %% Data
df = pd.read_csv("/ceph/lfertig/Thesis/data/processed/df_helpdesk.csv.gz")
df["time:timestamp"] = pd.to_datetime(df["time:timestamp"])
df = df.sort_values(by=["case:concept:name", "time:timestamp"]).reset_index(drop=True)

# Build RT prefixes:
# For each case with events at times t_0..t_{n-1}, end_time = t_{n-1}
# For i in 1..n-1: prefix = acts[:i], remaining_time = end_time - t_{i-1}  (days)
rows = []
for case_id, g in df.groupby("case:concept:name", sort=False):
    ...
    for i in range(1, len(acts)):
        ...
        rows.append({
            "case_id": case_id,
            "prefix": acts[:i],
            "remaining_time": (end_time - last_time).total_seconds() / 86400.0,
            "k": i
        })
rt_df = pd.DataFrame(rows)

# Temporal split by case start (as in your other baselines)
case_start = (
...
GLOBAL_MEAN = float(train_df["remaining_time"].mean()) if len(train_df) else float("nan")
wandb.log({"global_mean_days_train": GLOBAL_MEAN})

# %% N-gram trainer (mean aggregation over remaining_time)
# Model format: dict[level_L][context_tuple] = {"sum": float, "count": int}
def fit_ngram_rt(df_part, n: int, min_coun: int):
    levels = {}  # build all levels 1..(n-1) for backoff
    for L in range(1, n):  # context length = L
...
def predict_remaining(prefix, levels, n: int, global_mean: float):
    """Backoff from (n-1) to 1; return mean remaining time if seen else global mean."""
...
def eval_per_k_regression(df_part, levels, n: int, global_mean: float):
    ...
    for k in range(1, max_k + 1):
        ...
        y_pred = np.array([predict_remaining(p, levels, n, global_mean) for p in subset["prefix"]], dtype=np.float32)
...
for n in config["n_values"]:
    levels_val = fit_ngram_rt(train_df, n, min_coun=config["min_coun"])
...
levels = fit_ngram_rt(train_df, n_final, min_coun=config["min_coun"])
...
# %% Plots → disk
plot_dir = "/ceph/lfertig/Thesis/notebook/HelpDesk/plots/Baselines/NGRAM/RT"
os.makedirs(plot_dir, exist_ok=True)

if len(k_vals):
    plt.figure(figsize=(8,5))
    plt.plot(k_vals, maes, marker='o', label='MAE')
    plt.title(f'RT (n-gram mean) — MAE vs. Prefix Length (k), n={n_final}')
    ...
    plt.title(f'RT (n-gram mean) — RMSE vs. Prefix Length (k), n={n_final}')
    ...
    plt.title(f'RT (n-gram mean) — MSE vs. Prefix Length (k), n={n_final}')
...
y_pred_all = np.array([predict_remaining(p, levels, n_final, GLOBAL_MEAN) for p in test_df["prefix"]], dtype=np.float32) \
...
if len(y_true_all):
    ...
    wandb.log({
        "scatter_true_vs_pred": wandb.plot.scatter(tab, "true_days", "pred_days", title="RT n-gram: True vs Pred (days)"),
...
for _, r in sample.iterrows():
    pred = float(predict_remaining(r["prefix"], levels, n_final, GLOBAL_MEAN))
...
# Convert contexts to strings using a safe separator
SEP = "||"
serializable = {}
...
model_dir = f"/tmp/ngram_rt_{ts}"
os.makedirs(model_dir, exist_ok=True)
model_path = os.path.join(model_dir, f"ngram_rt_n{n_final}.json")
...
artifact = wandb.Artifact(name=f"ngram_rt_artifacts_{ts}", type="model")
