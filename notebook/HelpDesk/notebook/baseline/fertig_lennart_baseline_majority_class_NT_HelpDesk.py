# %% Majority Baseline — Next Time (NT)
# - Predict a single constant Δt (in days) from TRAIN (median or mean).
# - Data & splits: same as your other scripts via data.loader (Task.NEXT_TIME).
# - Metrics: per-k MAE/MSE/RMSE (days) + macro averages; global scatter + error histogram.
# - Logging: W&B curves/tables; headless matplotlib plots saved to disk.
...
run = wandb.init(
    project=f"baseline_majority_NT_{config['dataset']}",
...
# %% Majority constant
train_days = train_df["next_time"].to_numpy(dtype=float) if len(train_df) else np.array([])
...
print(f"Global constant NT (train, days) [{config['constant_choice']}]: {constant_days:.4f}")
wandb.log({"constant_nt_days_train": constant_days})

# %% Per-k evaluation (days)
k_vals, counts, maes, mses, rmses = [], [], [], [], []

for k in sorted(test_df["k"].astype(int).unique()) if len(test_df) else []:
...
# %% Plots → disk (days)
plot_dir = f"/ceph/lfertig/Thesis/notebook/{config['dataset']}/plots/Baselines/MAJ/NT"
...
# %% Log curves + macro metrics to W&B (same keys as TabPFN NT)
wandb.log({
    "curves/k": k_vals,
...
y_true_all = test_df["next_time"].to_numpy(dtype=float) if len(test_df) else np.array([])
y_pred_all = np.full_like(y_true_all, constant_days, dtype=float)
...
for _, r in sample.iterrows():
    ...
    print("-"*60)
    table.add_data(r["case_id"], int(r["k"]), prefix_pretty, gold, pred, abs(gold - pred))
