# %% Zero-shot Next-Time (NT) with Qwen3-4B — chat template + quantile bins + priors
...
config = {
    # bookkeeping
    "dataset":                  DATASET,
    "plots_dir":                f"/ceph/lfertig/Thesis/notebook/{DATASET}/plots/Qwen3-4B/ZS/NT",
    "unit":                     "days"
}
...
run = wandb.init(
    project=f"Qwen3-4B_NT_ZeroShot_{config['dataset']}",
    entity="privajet-university-of-mannheim",
    name=f"qwen3-4b_zs_nt_{ts}",
...
# %% Build quantile bins on TRAIN (NT in days) → labels
...
def score_bins(prefix_tokens, last_act=None):
    P_ids = get_prompt_ids(prefix_tokens).unsqueeze(0)  # [1, T]
    ...
    for lbl in BIN_LABELS:
        ...
        for t in range(tgt.size(1)):
            o = model(input_ids=prev, past_key_values=cur_past, use_cache=True)
            logp = torch.log_softmax(o.logits[:, -1, :], dim=-1)
            s += logp.gather(-1, tgt[:, t:t+1]).sum()
...
for _, r in test_df.iterrows():
    # top-1 bin = argmax score
    scores, probs = score_bins(r["prefix"], r["last_act"])
