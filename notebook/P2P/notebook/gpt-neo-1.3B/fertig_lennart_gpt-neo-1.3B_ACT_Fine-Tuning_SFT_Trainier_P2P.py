...
run = wandb.init(
    project="gpt-neo-1.3B_ACT_Fine-Tuning_SFT_Trainer_P2P",
...
load_kwargs = dict(torch_dtype=(torch.float16 if use_cuda else torch.float32))
print(f"Loading base model in dtype={load_kwargs['torch_dtype']} (no 4-bit).")
...
collator = CompletionOnlyCollatorRobust(tokenizer, response_template="Assistant:", max_length=256)
...
for n, p in m.named_parameters():
    if "lora_" in n:
        g = None if p.grad is None else float(p.grad.abs().mean().float().item())
        seen.append((n, p.requires_grad, g))
        if len(seen) >= 6: break

print("One-batch loss:", float(loss))
for n, rg, g in seen:
    print(" ", n, "requires_grad:", rg, "grad_mean:", g)
...
y_true_all = [_norm(x) for x in test_df["next_activity"].tolist()]
y_pred_all = [_norm(predict_next_scoring(p)[0]) for p in test_df["prefix"]]
...
# %%
plot_dir = "/ceph/lfertig/Thesis/notebook/P2P/plots/FT/ACT"
