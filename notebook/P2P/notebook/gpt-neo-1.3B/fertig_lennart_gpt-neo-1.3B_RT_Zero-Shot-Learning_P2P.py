# %% Zero-shot Remaining-Time (RT) with GPT-Neo-1.3B — quantile bins + priors
...
# %% Build quantile bins on TRAIN (RTin days) → labels
...
model = AutoModelForCausalLM.from_pretrained(
    BASE_MODEL, low_cpu_mem_usage=True, torch_dtype=DTYPE
).to(DEVICE).eval()
...
# Prompt builder
labels_for_prompt = "\n".join(BIN_LABELS)
def build_prompt(prefix_tokens, elapsed_days=None):
    t = prefix_tokens[-ZS_CFG["ctx_events"]:]
    trace = ZS_CFG["event_sep"].join(t)
    maybe_elapsed = ""
    if elapsed_days is not None and not (isinstance(elapsed_days, float) and np.isnan(elapsed_days)):
        maybe_elapsed = f"Elapsed so far: {float(elapsed_days):.3f} days\n"
    return ZS_CFG["prompt_tmpl"].format(
        trace=trace, labels=labels_for_prompt, maybe_elapsed=maybe_elapsed
    )
...
def get_prompt_ids(prefix_tokens, elapsed_days=None):
    ...
    prompt = build_prompt(prefix_tokens, elapsed_days)
...
def score_bins(prefix_tokens, last_act=None, elapsed_days=None):
    ...
    P_ids = get_prompt_ids(prefix_tokens, elapsed_days)
