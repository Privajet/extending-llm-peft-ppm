# %% Fine-tuned Remaining-Time (RT) with GPT-Neo-1.3B — LoRA + SFT
# - Cast RT regression to classification over TRAIN-derived quantile bins (days).
...
# - Tokenizer left-truncation; prompts list all time-bin labels; label text is the gold target.
# - Inference: score each bin by log-likelihood of [prompt + bin_label], softmax to bin probs,
#   then decode numeric RT as expected value over bin centers; also return top-k bins.
...
def predict_time(prefix, last_act=None, elapsed_days=None, return_topk=3):
