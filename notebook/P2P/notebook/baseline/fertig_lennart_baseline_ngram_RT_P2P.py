...
run = wandb.init(
    project=f"baseline_ngram_RT_{config['dataset']}",
...
def to_tokens(prefix_cell):
    # Prefixes are typically stored as space-joined strings
    if isinstance(prefix_cell, list):
        return [str(t) for t in prefix_cell]
    return str(prefix_cell).strip().split()

def fit_ngram_rt(df_part: pd.DataFrame, n: int, min_count: int):
...
def predict_remaining(prefix_tokens, levels, n: int, global_mean: float):
...
for n in config["n_values"]:
    ...
    y_pred_val = np.array(
        [predict_remaining(to_tokens(p), lvls, n, GLOBAL_MEAN) for p in val_df["prefix"]],
        dtype=float
    )
    val_mae = metrics.mean_absolute_error(y_true_val, y_pred_val)
...
# %% Plots → disk (days)
plot_dir = f"/ceph/lfertig/Thesis/notebook/{config['dataset']}/plots/Baselines/NGRAM/RT"
