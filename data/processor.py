...
class LogsDataProcessor:
    def __init__(self, name, filepath, columns, dir_path = "./data/processed", pool = 1):
    ...
    def _load_df(self, sort_temporally = False):
        ...
        # robust parse incl. ISO8601 with timezone like "2012-10-25 11:54:26+00:00"
        ts = pd.to_datetime(df["time:timestamp"], utc=True, errors="coerce")
        ts = ts.dt.tz_localize(None)  # drop timezone -> naive
    ...
    def _next_time_helper_func(self, df):
        ...
        for _, case in enumerate(unique_cases):
            ...
            time = df[df[case_id] == case][event_time].dt.strftime("%Y-%m-%d %H:%M:%S").to_list()
            time_passed = 0
    ...
    def _process_next_time(self, df, train_list, test_list, val_list):
        df_split = np.array_split(df, self._pool)
        with Pool(processes=self._pool) as pool:
        ...
        # val_df.to_csv(f"{self._dir_path}/{Task.NEXT_TIME.value}_val.csv", index = False)
    ...
    def _process_remaining_time(self, df, train_list, test_list, val_list):
        df_split = np.array_split(df, self._pool)
        with Pool(processes=self._pool) as pool:
        ...
        val_remaining_time = processed_df[processed_df["case_id"].isin(val_list)]
        ...
        val_remaining_time.to_csv(f"{self._dir_path}/{Task.REMAINING_TIME.value}_val.csv", index = False)

    def process_logs(self, task, sort_temporally = True,
        train_test_ratio = 0.80):
        df = self._load_df(sort_temporally)
        self._extract_logs_metadata(df)
        # Explicit temporal case order by *case start*
        case_start = (df.groupby("case:concept:name")["time:timestamp"].min().sort_values(kind="mergesort"))
        case_ids = case_start.index.tolist()
        n_total = len(case_ids)
        n_train = int(n_total * 0.8)
        n_val   = int(n_train * 0.1)           # -> 80/10/10 split
        train_list = case_ids[:n_train - n_val]
        val_list   = case_ids[n_train - n_val:n_train]
        test_list  = case_ids[n_train:]
        if task == Task.NEXT_ACTIVITY:
            self._process_next_activity(df, train_list, test_list, val_list)
        elif task == Task.NEXT_TIME:
            self._process_next_time(df, train_list, test_list, val_list)
        elif task == Task.REMAINING_TIME:
            self._process_remaining_time(df, train_list, test_list, val_list)
        else:
